	...
	@if(session('cart'))
		<div class="container pt-3 mb-9">
			...
			<div class="float-right">
				<a href="{{ route('adminindex')}}" class="text-light btn btn-primary">Add Product</a>
				<a href="{{ route('cart.alldelete')}}" OnClick="return confirm('Are You Sure to Delete All');" class="text-light btn btn-danger">Empty Cart</a>
				<a href="{{ route('admincheckoutform') }}"><button class="btn btn-success">Check Out</button></a>
			</div>
		</div>
	@else
	<div class="container text-center shadow p-5 mb-5">
		<h1 class="text-danger">Your Cart is Empty</h1>
		<a href="{{ route('adminindex') }}" class="text-light btn btn-success">Continute Shopping</a>
	</div>
	@endif
</div>
